...
import { assignCase } from '@/lib/api';
...
export default function CaseDetailPage() {
  ...
  const statusPanelStyles: Record<string, { container: string; iconBg: string; iconText: string; icon: string }> = {
    ...
    READY_FOR_CASEWORKER_REVIEW: { container: 'bg-blue-50 border-blue-400', iconBg: 'bg-blue-500', iconText: 'text-white', icon: '📋' },
  };
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <AssignedToField
                ...
                onAssign={async () => {
                  ...
                  setAssigning(true);
                  try {
                    await assignCase(caseData.id, user.id, user.name);
          ...
          <div className="bg-fast-panel rounded-lg shadow-card p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-fast-text whitespace-pre-line">
                  {caseData.aiSummary
                    ? caseData.aiSummary
              ...
              {caseData.extractedData && Object.keys(caseData.extractedData).length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-fast-muted uppercase mb-2">Extracted Data</p>
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(caseData.extractedData).map(([key, val]) => (
                      <div key={key} className="p-2 bg-gray-50 rounded-md">
                        <span className="text-xs text-fast-muted">{key.replace(/_/g, ' ')}</span>
                        <p className="text-sm font-medium text-fast-text">{val != null ? String(val) : '—'}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {(caseData.ruleEvaluations && caseData.ruleEvaluations.length > 0) ? (
                    caseData.ruleEvaluations.map((item, idx) => (
                      <div key={item.ruleId || idx} className="p-2 rounded-md bg-gray-50">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-fast-text font-medium">
                            {item.rule || item.ruleId || `Rule ${idx + 1}`}
                          </span>
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                            item.passed ? 'bg-fast-green-light text-fast-approved' :
                            item.status === 'INCONCLUSIVE' ? 'bg-yellow-100 text-yellow-800' :
                            'bg-fast-red-light text-fast-declined'
                          }`}>
            ...
            <ol className="relative border-l border-gray-200 ml-2 space-y-4">
              {(caseData.auditTrail && caseData.auditTrail.length > 0) ? (
                caseData.auditTrail.map((entry, i) => (
                  <li key={i} className="ml-4">
                    ...
                    <p className="text-xs text-fast-muted">
                      {entry.eventAt ? formatDate(entry.eventAt) : '—'} — {entry.agent || 'System'}
                    </p>
                  </li>
                ))
              ) : (
                <>
                  <li className="ml-4">
                    <div className="absolute -left-1.5 mt-1 w-3 h-3 rounded-full bg-fast-teal border-2 border-white" />
                    <p className="text-sm font-medium text-fast-text">Case Submitted</p>
                    <p className="text-xs text-fast-muted">{formatDate(caseData.createdAt)} — Applicant</p>
                  </li>
                  <li className="ml-4">
                    <div className="absolute -left-1.5 mt-1 w-3 h-3 rounded-full bg-fast-teal border-2 border-white" />
                    <p className="text-sm font-medium text-fast-text">Status: {caseData.status.replace(/_/g, ' ')}</p>
                    <p className="text-xs text-fast-muted">{formatDate(caseData.updatedAt)} — System</p>
                  </li>
                </>
              )}
            </ol>
...
function AssignedToField({
  ...
  return (
    <div>
      <p className="text-xs text-fast-muted mb-0.5">Assigned To</p>
      <div className="flex items-center gap-2">
        {isUnassigned ? (
          <>
            <span className="text-sm text-fast-muted italic">Unassigned</span>
            <button
              onClick={onAssign}
              disabled={assigning}
              className="text-xs font-medium text-white bg-fast-teal px-2.5 py-1 rounded-md hover:opacity-90 transition-opacity disabled:opacity-50"
            >
          ...
          <>
            <span className="text-sm font-medium text-fast-approved">{currentUserName} (you)</span>
          </>
        ) : (
          <>
            <span className="text-sm font-medium text-fast-text">{assignedToName}</span>
            <button
              onClick={onAssign}
              disabled={assigning}
              className="text-xs font-medium text-fast-teal border border-fast-teal/40 px-2.5 py-1 rounded-md hover:bg-fast-teal/5 transition-colors disabled:opacity-50"
            >
              {assigning ? 'Reassigning…' : 'Reassign to me'}
            </button>
          </>
        )}
      </div>
