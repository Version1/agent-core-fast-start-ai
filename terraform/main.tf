...
locals {
  ...
  # On Windows the default local-exec interpreter is cmd.exe, which cannot run
  # the bash scripts below. Detect the OS (abspath is drive-prefixed on Windows,
  # "/"-rooted elsewhere) and route bash provisioners through Git Bash.
  is_windows             = substr(abspath(path.root), 0, 1) != "/"
  local_exec_interpreter = local.is_windows ? ["C:/Program Files/Git/bin/bash.exe", "-c"] : ["/bin/bash", "-c"]
}
...
module "aurora" {
  ...
  lambda_request_role_arn = module.iam.lambda_request_role_arn
}
...
module "lambda" {
  ...
  lambda_roles = {
    request_arn = module.iam.lambda_request_role_arn
    ai_arn     = module.iam.lambda_ai_role_arn
    admin_arn   = module.iam.lambda_admin_role_arn
  }
...
module "cognito" {
  ...
  callback_urls = concat(
    var.github_access_token != "" ? ["https://www.${var.domain_name}/api/auth/callback"] : [],
    var.cognito_extra_callback_urls
  )

  logout_urls = concat(
    var.github_access_token != "" ? ["https://www.${var.domain_name}"] : [],
...
# Without a token, run the frontend locally: cd ui && npm run dev
################################################################################
...
module "amplify" {
  ...
  github_repository   = "https://github.com/shajida95/fastart-ai-build"
  github_branch       = "main"
...
module "waf_regional" {
  source = "./modules/waf_regional"

  name         = "case-triage-waf-regional"
  resource_arn = "arn:aws:apigateway:${local.region}::/restapis/${module.api_gateway.api_id}/stages/${module.api_gateway.stage_name}"
}
...
module "waf" {
  ...
  name            = "case-triage-waf-global"
  amplify_app_arn = module.amplify[0].app_arn
}
