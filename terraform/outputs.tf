...
output "amplify_hostname_for_cognito" {
  description = "Set variable amplify_app_domain to this value (hostname only) then terraform apply, so Cognito allows OAuth redirect back to the portal"
  value       = length(module.amplify) > 0 ? replace(module.amplify[0].default_domain, "https://", "") : ""
}
...
output "cognito_hosted_ui_url" {
  description = "Full Cognito Hosted UI base URL (authorize endpoint is under /oauth2/authorize)"
