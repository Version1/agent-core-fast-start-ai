"""
PUT /cases/{caseId}/assign - Assign or unassign a case.
Body: assignedTo (user ID), assignedToName.
If assignedTo is empty/missing, unassigns the case.
...
def lambda_handler(event, context):
    ...
    is_unassign = not assigned_to

    try:
        table = dynamodb.Table(TABLE)

        if is_unassign:
            # Unassign: remove assignedTo and assignedToName
            table.update_item(
                Key={"caseId": case_id},
                UpdateExpression="REMOVE assignedTo, assignedToName SET updatedAt = :now",
                ExpressionAttributeValues={":now": now},
            )

            _rds_execute(
                """UPDATE cases SET assigned_to = NULL, assigned_to_name = NULL, updated_at = :now WHERE case_id = :cid""",
                [_rds_param("now", now), _rds_param("cid", case_id)],
            )
            ...
            audit_table.put_item(Item={
                ...
                "details": json.dumps({"caseId": case_id}),
                "expiresAt": ttl,
            })

            return _response(200, {"caseId": case_id, "assignedTo": "", "assignedToName": ""})
