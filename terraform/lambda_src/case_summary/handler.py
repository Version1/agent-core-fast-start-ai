...
def lambda_handler(event, context):
    ...
    confidence_label = summary.get("data_quality_assessment", {}).get("overall_confidence", "MEDIUM").upper()
    CONFIDENCE_MAP = {"HIGH": 85, "MEDIUM": 60, "LOW": 35}
    ai_confidence = CONFIDENCE_MAP.get(confidence_label, 60)

    policy_result = safe_json(case.get("policyResult", "{}"))
    overall_policy = policy_result.get("overall_status", "")
    if overall_policy == "ELIGIBLE":
        ai_recommendation = "APPROVE"
    elif overall_policy == "INELIGIBLE":
    ...
    if ai_recommendation:
        extra["aiRecommendation"] = ai_recommendation
    update_status(case_id, "READY_FOR_CASEWORKER_REVIEW", extra)
