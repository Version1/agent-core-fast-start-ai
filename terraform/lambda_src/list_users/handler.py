    ...
    try:
        ...
        try:
            ...
            for row in rows:
                if not row:
                    continue
                row_email = row.get("email", "") if isinstance(row, dict) else ""
                if row_email:
                    caseworkers_by_email[row_email] = {
                        "caseworker_id": row.get("caseworker_id", ""),
                        "department": "",
                        "role": row.get("role_name", "") if isinstance(row, dict) else "",
        ...
        try:
            rows = _aurora_execute(
                "SELECT assigned_caseworker::text AS cw_id, COUNT(*) AS cnt FROM cases "
                "WHERE assigned_caseworker IS NOT NULL GROUP BY assigned_caseworker"
            )
            for row in rows:
                if not row:
                    continue
                cw_id = str(row.get("cw_id", "")) if isinstance(row, dict) else ""
                count = row.get("cnt", 0) if isinstance(row, dict) else 0
                if cw_id:
        ...
        for u in cognito_users:
            ...
            full_name = _get_attr(attrs, "name")
            given = _get_attr(attrs, "given_name")
            family = _get_attr(attrs, "family_name")
            name = full_name or f"{given} {family}".strip() or email or username
            ...
            department = _get_attr(attrs, "custom:department") or cw.get("department", "")
