...
variable "ses_sender_email" { default = "noreply@casehubportal.com" }
...
resource "aws_lambda_function" "app_init" {
  ...
  role             = var.lambda_roles.request_arn
...
resource "aws_lambda_function" "list_users" {
  ...
  role             = var.lambda_roles.admin_arn
  handler          = "handler.lambda_handler"
  runtime          = local.runtime
  timeout          = 30
  memory_size      = 256
