  ...
  return (
    <div className="p-6 flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 min-w-0">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-2xl font-semibold text-fast-text">Settings</h1>
          </div>
          <p className="text-sm text-fast-muted mt-1">Manage your account and application preferences</p>
        </div>

        {/* Profile Information */}
        <section className="bg-fast-panel rounded-card border border-gray-200 overflow-hidden mb-4">
          <button
            type="button"
          ...
          >
            ...
            {profileOpen ? <ChevronDown className="w-4 h-4 text-fast-muted" /> : <ChevronRight className="w-4 h-4 text-fast-muted" />}
          </button>
          {profileOpen && (
            <div className="px-5 pb-5 pt-0 border-t border-gray-100">
              <div className="flex flex-wrap items-start gap-6 mb-6">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-2xs text-fast-muted">JPG, PNG or GIF. Max 2MB</p>
                </div>
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-[200px]">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      ...
                      readOnly
                      value={user?.email ?? ''}
                      className="w-full px-3 py-2 border border-gray-200 rounded text-sm bg-gray-50 text-fast-muted"
                    />
                    <p className="text-2xs text-fast-muted mt-1">Email cannot be changed. Contact IT for assistance.</p>
                  </div>
                  <div>
        ...
        <section className="bg-fast-panel rounded-card border border-gray-200 overflow-hidden mb-4">
          ...
          >
            ...
            {notifOpen ? <ChevronDown className="w-4 h-4 text-fast-muted" /> : <ChevronRight className="w-4 h-4 text-fast-muted" />}
          </button>
          {notifOpen && (
        ...
        {/* Display Preferences */}
        <section className="bg-fast-panel rounded-card border border-gray-200 overflow-hidden">
          <button
            type="button"
          ...
          >
            ...
            {displayOpen ? <ChevronDown className="w-4 h-4 text-fast-muted" /> : <ChevronRight className="w-4 h-4 text-fast-muted" />}
          </button>
          {displayOpen && (
            <div className="px-5 pb-5 pt-0 border-t border-gray-100 space-y-3">
              <label className="flex items-center gap-3">
      ...
      <aside className="w-full lg:w-80 flex-shrink-0">
        <div className="bg-fast-panel rounded-card border border-gray-200 p-5 mb-4">
