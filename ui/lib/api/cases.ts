...
export async function unassignCase(
  ...
  const { data } = await apiClient.put<{ caseId: string; assignedTo: string; assignedToName: string }>(
    `/cases/${caseId}/assign`,
    { assignedTo: '', assignedToName: '', performedByName: performedByName || '' },
  );
  return data;
}
...
export async function addCaseNote(
  ...
  note: string,
  authorName: string,
): Promise<{ caseId: string; note: { text: string; author: string; timestamp: string } }> {
  const { data } = await apiClient.put<{ caseId: string; note: { text: string; author: string; timestamp: string } }>(
    `/cases/${caseId}/assign`,
    { note, authorName },
  );
  return data;
